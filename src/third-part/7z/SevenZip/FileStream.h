/*******************************************************************************
  ...
  这次主要去掉了关键的业务协议相关的东西，将dataset拿掉，去掉了xml拿掉,简化ERPBlob为MemoryStream
...
class  ParamObject
{
	protected:
		...
		MemoryStream mParamsBuffer;            //参数字节流
        unsigned int curr_pos;                  //当前指针位置，此参数只用在取数时，填数时决不能用！！
    ...
    public:
        ...
        //从Blob对象中反序列化
		void LoadFromBlob(MemoryStream &);

        //序列化到Blob对象中
		void SaveToBlob(MemoryStream &);
    ...
    public:         
		...
		void putBlob(const char *paramName, MemoryStream &blob);
		...
		bool getBlob(const char *paramName, MemoryStream &blob, bool bException=true);
		unsigned char *getBlobEx(const char *paramName, unsigned int  &length);

        void First();                           //当前参数指向第一个参数
        void Next();                            //当前参数指向下一个参数
        int Eof();                             //是否已经到最后一个参数
        std::string getParamName();                  //得到当前参数的名称
        int getParamType();  //得到当前参数的类型
        int getParamValue(unsigned char **value);      //得到当前参数的值
        void getParamValue(MemoryStream *pBlob,unsigned char fillEndChar);
        std::string getParamValue();  //得到当前的参数的字符串值，如果当前参数是流就返回空
        void InitParamNames(); //初始化名字，重建索引
        void WriteLog(const char *file_name,const char *open_flag,const char *head_title,const char *text=NULL);
}; 
...
public:
	...
	~CFileStream();
public:
	bool PutFile(const char *fileName, MemoryStream &blob);            //加入文件
	bool GetFile(const char *fileName, MemoryStream &blob);            //加入文件
