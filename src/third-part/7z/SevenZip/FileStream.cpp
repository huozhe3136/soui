...
void ParamObject::putString(const char *paramName,const char *paramValue,unsigned char dataType/*=PARAMS_STRING*/)
{
      ...
      putStream(paramName,(const unsigned char *)paramValue,(unsigned int)strlen(paramValue),false,dataType);
}
...
void ParamObject::putBlob(const char *paramName,MemoryStream &blob)
{
...
void ParamObject::putDataSet(const char *paramName,DataSet &dataset)
{
      ...
      dataset.Serialize(MemoryStream) ;

      putStream( paramName,MemoryStream.GetBlobPtr(),MemoryStream.GetBlobLength(),true,PARAMS_DATASET );
}
*/
//支持参数对象嵌套
...
{
      MemoryStream MemoryStream;
	  object.SaveToBlob(MemoryStream);
      putStream( paramName,MemoryStream.GetBlobPtr(),MemoryStream.GetBlobLength(),true,PARAMS_ParamObject );
}
...
bool ParamObject::getBlob(const char *paramName, MemoryStream &blob,bool bException)
{ 
...
{
     return(atof(getString(paramName,PARAMS_FLOAT).c_str()));
}
      ...
      if(getParamPos(paramName,&stream,len,NodeType,PARAMS_ParamObject))
      {
          if(PARAMS_ParamObject==NodeType)
          {
                ...
                { 
                    ...
                    temptime= time(0);
                    temptm = localtime(&temptime);
                    sprintf(c_time,"%d%02d%02d %02d:%02d:%02d",
                           temptm->tm_year+1900,temptm->tm_mon+1,temptm->tm_mday,
                           temptm->tm_hour,temptm->tm_min,temptm->tm_sec);
                    sprintf(c_time,"%d%02d%02d%02d%02d%02d_exception.log",
                            temptm->tm_year+1900,temptm->tm_mon+1,temptm->tm_mday,
                            temptm->tm_hour,temptm->tm_min,temptm->tm_sec );
                    SaveToFile(c_time);
                    throw e;
...
int ParamObject::getParamValue(unsigned char **value)
{
      ...
      if(pNode->length<1)
      {
       throw ERPException(EERROR,SYSTEM_ERROR,"该参数不存在！");
      }
...
void ParamObject::getParamValue(MemoryStream *pBlob,unsigned char fillEndChar)
{
     ParamNode *pNode;
      pNode = (ParamNode *)( (unsigned char*)mParamsBuffer.GetBlobPtr()+curr_pos );
      if(pNode->length<1)
...
{
     return mParamsBuffer.GetMemoryStreamLength();
}
//clone
...
//从Blob对象中反序列化
void ParamObject::LoadFromBlob(MemoryStream &blob)
{
    LoadFromStream(blob.GetBlobPtr(),blob.GetBlobLength());
}

//序列化到Blob对象中
void ParamObject::SaveToBlob(MemoryStream &blob)
{
...
{
      return (unsigned int)paramNames.size();
}
...
//加入文件
bool CFileStream::PutFile(const char *fileName, MemoryStream &blob)
{ 
...
//加入文件
bool CFileStream::GetFile(const char *fileName, MemoryStream &blob)
{
	//改为不区分大小写吧，soui ui库大小写比较乱．．.	  
	std::string file_name(fileName);
	file_name = tolowerstring(file_name);
