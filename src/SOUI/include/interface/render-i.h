...
namespace SOUI
{
    ...
    */
    ...
    { 
         ...
         * Describe  
         */    
        virtual  LPVOID GetPixelBits() const = 0;
         ...
         */    
        virtual HRESULT Clone(IBitmap **ppClone) const = 0;
         ...
         * Describe  成功后，调用者执行IBitmap::Release
         */
		 virtual HRESULT Scale(IBitmap **pOutput,int nScale,FilterLevel filterLevel) = 0;
         ...
         * Describe  成功后，调用者执行IBitmap::Release
         */
		virtual HRESULT Scale(IBitmap **pOutput,int nWid,int nHei,FilterLevel filterLevel)=0;
         ...
         * Describe  
         */    
        virtual HRESULT Save(LPCWSTR pszFileName,const LPVOID pFormat) = 0;
    };
    ...
    */
	...
	struct IPath : IRenderObj
	{
		...
		*/
		virtual void setFillType(FillType ft) =0;
		...
		*/
		virtual void toggleInverseFillType() =0;
		...
		*/
		virtual void setConvexity(Convexity c) =0;
		...
		*/
		virtual bool isOval(RECT* rect) const =0;
		...
		*/
		virtual bool isRect(RECT* rect) const =0;
		...
		*/
		virtual int getPoints(POINT points[], int max) const =0;
		...
		*/
		virtual int getVerbs(BYTE verbs[], int max) const =0;
		...
		*/
		...
		previous contour, to specify the start of a new contour
		*/
		virtual void rMoveTo(float dx, float dy) =0;
		...
		*/
		...
		on this contour, to specify a line
		*/
		virtual void rLineTo(float dx, float dy) =0;
		...
		*/
		...
		this contour, to specify the end point of a quadratic curve
		*/
		virtual void rQuadTo(float dx1, float dy1, float dx2, float dy2)=0;
		...
		*/
		...
		@return true if the path specifies a rectangle
		*/
		virtual bool isRect(bool* isClosed, Direction* direction) const =0;
		...
		*              kUnknown_Direction.
		*/
		virtual void addRect(const RECT& rect, Direction dir = kCW_Direction)=0;
		...
		*              kUnknown_Direction.
		*/
		virtual void addOval(const RECT& oval, Direction dir = kCW_Direction)=0;
		...
		*/
		virtual void addArc(const RECT& oval, float startAngle, float sweepAngle)=0;
		...
		*      }
		*/
		virtual void addPoly(const POINT pts[], int count, bool close) = 0;
		...
		*/
		virtual void addPath(const IPath * src, float dx, float dy,
			AddPathMode mode = kAppend_AddPathMode) = 0;


		/**
		*  Same as addPath(), but reverses the src input
		*/
		virtual void reverseAddPath(const IPath* src) = 0;
		...
		*/
		virtual void offset(float dx, float dy) =0;
		...
		*/
		virtual bool getLastPt(POINT* lastPt) const = 0;
		...
		*/
		virtual void setLastPt(float x, float y) = 0;
