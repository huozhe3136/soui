...
namespace SOUI
{
	...
	SkinKey::SkinKey(const SkinKey & other)
	{
		this->strName = other.strName;
		this->scale   = other.scale;
	}

	SStylePool::SStylePool()
	{
...
SSkinPool::~SSkinPool()
{
    ...
    while(pos)
    {
        ...
        {
            STRACEW(L"skin of [%s.%d] was not used.",(const wchar_t *)skinKey.strName, (const wchar_t *)skinKey.scale);
        }
    }
...
int SSkinPool::LoadSkins(pugi::xml_node xmlNode)
{
    ...
    while(xmlSkin)
    {
        ...
        {
            SASSERT_FMTW(false,L"load skin error,type=%s,name=%s",(const wchar_t *)strTypeName, (const wchar_t *)strSkinName);
        }
        xmlSkin=xmlSkin.next_sibling();
    }
...
{
	for (unsigned int i = 1; i < ARRAYSIZE(KBuiltinScales); i++)
	{
...
ISkinObj* SSkinPool::GetSkin(const SStringW & strSkinName,int nScale)
{
    ...
    if(!HasKey(key))
    {
		...
		{
			bool bFind = false;
			for (unsigned int i = 0; i < ARRAYSIZE(KBuiltinScales); i++)
			{
...
ISkinObj* SSkinPoolMgr::GetSkin( const SStringW & strSkinName, int nScale)
{
    ...
    {
        SASSERT_FMTW(FALSE,L"GetSkin[%s] Failed!",(const TCHAR *)strSkinName);
    }
    return NULL;
}
